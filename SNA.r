...
setwd("C:/Users/feder/Desktop/BLOCKCHAIN")
...
edges<- read.csv("https://raw.githubusercontent.com/federico-jf/blockchain-education/main/edges_blockchain.csv", header=T, as.is=T)
nodes<- read.csv("https://raw.githubusercontent.com/federico-jf/blockchain-education/main/nodes_blockchain.csv", header=T, as.is=T)


networkasigraph <- graph_from_data_frame(d=edges, vertices=nodes, directed = FALSE)
networkasigraph
...
# OLD PROJECT mean(nodes$number_tweets)
...
# OLD PROJECT statistics for other variables
...
plot(networkasigraph, layout=layout_nicely,
     ...
     vertex.label=V(networkasigraph)$id_label,
     main="Title")
